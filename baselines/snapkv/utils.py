...
class SnapKVCluster():
    def __init__(self, window_size = 64, max_capacity_prompt = 256 + 64, kernel_size = 5,
     pooling = 'avgpool', merge = None, recent_size = 32, ratio =  0.4,
     retain_rate=0.25, eviction_mode="constant", dataset = None):
    ...
    def update_kv(self, key_states, query_states, value_states, attention_mask, num_key_value_groups, layer_idx):
        ...
        assert repeated_key_states.shape[-2] == query_states.shape[-2]
        ...
        if q_len < self.max_capacity_prompt:
            ...
            attn_weights = torch.matmul(query_states[..., -self.window_size:, :], repeated_key_states.transpose(2, 3)) / math.sqrt(head_dim)
            ...
            origin_attn_weights = attn_weights

            attn_weights = attn_weights.view(bsz, -1, num_key_value_groups, self.window_size, attn_weights.size(-1)).sum(dim=2)
